...
(
    ...
    matching_status     VARCHAR(30) NOT NULL CHECK (matching_status IN ('MATCHED', 'FAILED', 'TBC')),
...
(
    ...
    salary_reserved_amount  DECIMAL(18, 2) NOT NULL,
    emergency_amount        DECIMAL(18, 2) NOT NULL,
    investment_amount       DECIMAL(18, 2) NOT NULL,
    living_amount           DECIMAL(18, 2) NOT NULL,
    distributed_at          TIMESTAMP      NOT NULL,

    CONSTRAINT fk_distribution_history_user
        FOREIGN KEY (user_id) REFERENCES USERS (user_id)
);
...
ALTER TABLE AI_CHAT_SESSION ADD COLUMN IF NOT EXISTS title VARCHAR(255);
ALTER TABLE AI_CHAT_SESSION ADD COLUMN IF NOT EXISTS status VARCHAR(20) NOT NULL DEFAULT 'ACTIVE' CHECK (status IN ('ACTIVE', 'CLOSED'));
ALTER TABLE AI_CHAT_SESSION ADD COLUMN IF NOT EXISTS created_at TIMESTAMP;
...
ALTER TABLE AI_CHAT_MESSAGE ADD COLUMN IF NOT EXISTS intent VARCHAR(50);
